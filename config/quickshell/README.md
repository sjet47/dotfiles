...
| 飞书（自带白色圆角方底） | 无变化——它自己的圆角比遮罩半径大 |
| 直角不透明方图 | **四角被裁圆**，这才是遮罩真正生效的场合 |
...
`pinnedMonitor`（默认 `DP-1`）。OSD 和电源菜单跟着焦点是对的——反馈应该出现在你正看的
那块屏上；但通知跟着焦点跳很烦，切个屏正在读的通知就跑了。

主屏不在（拔了、改名）时回退到焦点屏，否则通知会静默消失。三种情况都实测过：
固定屏存在时压过焦点、固定屏改成另一块时通知确实跟着走、固定屏填不存在的名字时回退。

### 左右两槽分别喂什么（实测，别照 spec 猜）

macOS 的语义是"应用图标在 leading edge、附件在 trailing edge"。映射到 D-Bus 的字段
**不能想当然**，实测三个来源：

| 来源 | `appIcon` | `image` |
| --- | --- | --- |
| `notify-send -i <路径>` | 空 | `image://icon/<路径>` |
| kitty | `/usr/lib/kitty/logo/kitty.png` | 空 |
| 飞书 | 自己的 logo 路径 | 空 |

也就是 `notify-send -i` 进的是 **`image`** 而不是 `appIcon` —— 跟名字给人的直觉相反。
好在这样分槽反而正好：`appIcon` 是应用自报的图标（kitty / 飞书）放左，
`image` 是外部传进来的图（截图预览）放右。

副作用：`notify-send` 发的通知左槽是空的，正文顶格。这是实情，不是漏做。
