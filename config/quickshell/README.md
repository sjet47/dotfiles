...
```
┌────────────────────────────────────────────┐
│ [应用图标]  标题（1 行）        [缩略图 或  │  高度跟内容
│  40x40     正文（最多 4 行）     竖排按钮]  │
└────────────────────────────────────────────┘
```

- **高度跟着内容长，不固定**，正文最多 4 行、下限 64。这是对着 macOS 实机截图定的——
  单行正文的横幅很扁、多行的明显长，并不定高，正文在第 4 行处省略号截断。
- **省略号有个边界**：Qt 的 `ElideRight` 只做**横向**省略，所以只有当第 4 行本身写满了
  才会出现 `…`。每行都很短（比如 `a<br/>b<br/>c<br/>d<br/>e`）时内容被静默切掉，
  没有省略号。真实的长句不受影响（实测飞书那种告警正常出现 `…`）。
- **右槽二选一**：有非 `default` 的 action 就放按钮（竖排，最多 2 个），否则放附件缩略图。
- 缩略图 56x56、**`PreserveAspectFit` 不能用 Crop** —— 截图是 16:9 的 4K 图，
  裁成正方形只会显示中心一小块，看不出截了什么。Fit 之后是一条横带，
  四角本来就透明，所以也不需要圆角遮罩。

### 应用图标是圆角裁剪的

左槽套了个 `MultiEffect` 圆角遮罩（`QtQuick.Effects`，半径 9）。但**对不同图标效果差别很大**，
实测三种：

| 图标 | 结果 |
| --- | --- |
| kitty（猫头，四周全透明） | 无变化——没有角可裁 |
| **飞书告警** | **四角被裁圆**——它发的不是 app logo 而是发信人头像，240x240 不透明 jpg |
| 直角不透明方图 | 同上，这才是遮罩真正生效的场合 |

遮罩**必须开抗锯齿**，`MultiEffect` 默认按 alpha 硬切，边缘只有一个像素过渡，圆角会是
肉眼可见的阶梯（12 倍放大对比过）。三处缺一不可：遮罩 Item 的 `layer.samples: 4`、
遮罩 Rectangle 的 `antialiasing: true`、以及 `maskThresholdMin: 0.5` + `maskSpreadAtMin: 1.0`
让它吃满遮罩自带的渐变。

顺带一个和布局有关的事实：**飞书通知的左槽是头像不是应用图标**（路径在
`LarkShell/sdk_storage/.../avatars/*_hd.jpg`），相当于 macOS 的 communication notification。

一开始试过"垫一块圆角底板 + 图标缩小居中"，能让透明图标也显出方形轮廓，但飞书那种
自带白底的会变成双层底板，所以否掉了。

### 悬停浮现的控件 + Options 弹出菜单

对着 macOS 实机截图做的：

- **静置时卡片是干净的** —— 没有 Options。
- **悬停**才浮现右下角的 `Options ⌄`。它是**覆盖层，不占布局**，会压在正文上
  （macOS 就是这样）。因为不占布局，附件缩略图和 action **可以同时存在**，
  不像早期版本那样二选一。
- 按钮周围的内容要**淡出**给它让位，否则按钮压在深色文字/缩略图上基本没法读。
  做法是给**整个内容层 `row`** 套一层透明度遮罩（`MultiEffect` + `maskSource`，
  白=保留、透明=擦掉），只在 `layer.enabled: optBtn.visible` 时生效。
  挂在 `row` 上而不是正文上 —— 右槽的附件缩略图同样会压在按钮底下，只遮正文的话有图时照旧。

  遮罩要的是"以按钮为中心向外 alpha 0→1"，但用不着 `QtQuick.Shapes` 的 `RadialGradient`：
  叠两道线性渐变就行（横向左白右透 + 纵向上白下透）。普通叠加是 over 合成，
  alpha 取的是**并集** `a2+a1(1-a2)`，所以只有"既靠右又靠下"的那个角两道都透明 ——
  正好是个软边角洞。

  **`MultiEffect` 的遮罩必须显式给 `maskThresholdMin: 0.5` + `maskSpreadAtMin: 1.0`**，
  否则渐变根本不成立。这两个参数是"围绕阈值的**半宽**"语义，`smoothstep(min-spread, min+spread, m)`：

  | 参数 | 实际映射 | 结果 |
  | --- | --- | --- |
  | 默认 `0.0 / 0.0` | 在 0 处硬阶跃 | alpha 被**二值化**，渐变压成一条直边，看着就是拿矩形切掉 |
  | `0.0 / 1.0` | `smoothstep(-1, 1, m)` | alpha 0 映射成 **0.5**，遮罩几乎失效，整行都擦不掉 |
  | `0.5 / 1.0` | `smoothstep(0, 1, m)` | 原样平滑透传 ✓（左边图标的圆角遮罩用的也是这一组） |

  排查手法：正文用 `y` 这种稀疏字形量不准（`minima` 只要窗口里有一个暗像素就报暗，
  会把渐变读成阶跃）。**用 `█` 实心方块当正文**，均值直接就是 alpha 剖面，一眼看穿。

  **渐变行程**当前横向 60px、纵向 26px。
  行程短了不像渐变，像直接盖了一块 —— 一开始给 44px / 22px 就是这个毛病。

  止点按 `optBtn` 的实际位置算，**不要硬编比例**：卡片高度随正文行数变（56 ~ 4 行），
  写死比例在单行卡片上会把右半边整片擦掉。`row` 的边距和按钮是对齐的
  （`row` 左右 margin 12 = 按钮 `rightMargin`，上下 8 = `bottomMargin`），
  所以按钮在 `row` 坐标里就是右下角那块 `optBtn.width × optBtn.height`。
- 按钮底色是 `#1a1d1d1f`（10% 的正文色 `#1d1d1f`），实测卡片 213 / 按钮 194 —— 轻微凹陷。
  背后的内容已经被遮罩擦干净，按钮不需要靠不透明度去挣可读性。
  **别反过来用"比卡片更实的浅色"**（试过 `#d9e9e9ed`）：卡片是半透明的，
  浅色叠上去合成结果**比卡片还亮**，按钮直接糊没（实测两边都是 213）。
- **弹出菜单要跟卡片同一种材质：同底色、同透明度**（`#d9f5f5f7` + `#d2d2d7` 描边）。
  原来是 `#f2f5f5f7`（95% 不透明）配黑色描边，比胶囊又亮又实，看着像另一块面板贴上来。
  菜单画在卡片外面、直接压桌面，所以照抄卡片的颜色合成出来就跟胶囊一模一样。

  **不能靠"在按钮底下垫一层卡片底色"** —— 试过，效果是错的：卡片本身是半透明的
  （`#d9f5f5f7`），再叠一层同色只会让那块比周围更亮，看着是个**高亮补丁**而不是擦除；
  想盖住文字就得比卡片更实（`#f2`），于是更亮、更显眼。加法永远减不掉不透明度，
  只有把文字本身遮掉才是真的擦。

  遮罩源 `Item` 用 `visible: false` + `layer.enabled: true`，直接绑 `width/height` 到被遮的项，
  别用 `anchors.fill` 指过去——跨父级会报 `Cannot anchor to an item that isn't a parent or sibling`。
- **点 Options 在卡片下方弹出独立菜单**，不是把卡片撑开。菜单项悬停高亮用 macOS 的蓝
  (`#0a84ff`) 配白字。
- 菜单画在卡片**外面**（下方），所以 `menu` 内部那个 `z: 100` **压不住下一条通知** ——
  卡片之间是 `stack`（Column）的兄弟节点，后面的卡片整体盖在前面的上面，菜单会被吃掉。
  要抬就得抬**整张卡片**：`z: card.menuOpen ? 1 : 0`。同一时刻只有一张卡片能开菜单
  （`notif.openMenuCard`），不会互相打架。
- **悬停或菜单打开时都停表**，离开后重新计时 —— 否则展开到一半通知就没了。

**hover 判定要用 `HoverHandler`，不要用 `MouseArea.containsMouse`。** 子 MouseArea 会从父的
手里抢走 hover，而且两者的状态更新**有先后**——光标离开 Options 那一帧 `opt` 已经变 false、
`card` 还没变 true，中间那一帧按钮就被判定为不该显示，于是在指针底下闪。

试过写成两者的并集（`cardMouse.containsMouse || optMouse.containsMouse`），**并集救不了时序缝隙**，
插桩日志里能直接看到：
...
`HoverHandler` 报告的是"指针在本 item 或任意子项之上"，不参与这场抢夺，也就没有缝隙。
以后再加悬停控件不用改判定。

**菜单打开时整块面板收输入**（`mask: notif.openMenuCard ? null : stackRegion`）。菜单画在
卡片外面，落在 `stack` 区域之外就点不到；顺带得到"点击别处关闭菜单"。

### 曾经的做法（都否掉了）

- 按钮常驻在右槽竖排：卡片矮时两个按钮挤在几十像素里很难看。
- 按钮切在卡片底部整宽分段：占掉固定高度，没有 action 的通知也得让出位置。
